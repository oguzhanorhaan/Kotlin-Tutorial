package com.oguzhanorhan.kotlinmqttexample.domain
...
class MQTTmanager(private val connectionParams: MQTTConnectionParams,val cntx: Context) {

    private var context : Context = cntx.applicationContext

    var uiUpdater: UIHandlerInterface? = null
    private var client = MqttAndroidClient(context,connectionParams.host,connectionParams.clientId )
    ...
    fun subscribe(topic: String){
        try
        {
            client.subscribe(topic, 0, null, object: IMqttActionListener {
                override fun onSuccess(asyncActionToken: IMqttToken) {
                    Log.w("Mqtt", "Subscription!+ ${uiUpdater}")
